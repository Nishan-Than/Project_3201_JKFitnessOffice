...
namespace ServiceLayer
{
    public class MemberShipService
    {
        ...
        public WebResponce GetMemberDetailsById(int memberId)
        {
            try
            {
                var memberdeatil = uow.MembershipRepository.GetByID(memberId);
                var membershipVM = new MembershipVM();

                if (memberdeatil != null)
                {
                    membershipVM.PackageType = uow.MembershipTypesRepository.GetByID(memberdeatil.MemberPackage).MembershipName;
                    membershipVM.PackageAmount = memberdeatil.Payment;
                    membershipVM.PackageExpirationDate = memberdeatil.PackageExpirationDate;

                    membershipVM.AllPaymentDetails = uow.MembershipPaymentsRepository.GetAll().Where(x => x.MemberId == memberId).OrderByDescending(x => x.PaymentDate).ToList();
