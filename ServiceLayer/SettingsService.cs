    ...
    public class SettingsService
    {
        ...
        public WebResponce GetUserRights(string UserType)
        {
            try
            {
                ...
                var RoleRights = uow.DbContext.MenusRights.Select(s => new
                {
                    Role = UserType == "Admin" ? s.Admin : UserType == "Temporary Staff" ? s.TemporaryStaff  : s.Staff
                }).ToList();
        ...
        public List<int> GetUserRightsbyUsertype(string UserType)
        {
            try
            {
                var menuRights = new List<int>();

                if (UserType == "Temporary Staff")
                    menuRights = uow.MenuRightsRepository.GetAll().Select(x => x.TemporaryStaff).ToList();
                else
                    menuRights = uow.MenuRightsRepository.GetAll().Select(x => UserType == "Admin" ? x.Admin : x.Staff).ToList();
